...
public class RuntimeTest {
    ...
    public static MyDate[][] generateTestdata() {
        final MyDate[][] array = new MyDate[2][TEST_SET_SIZE];
        final Random r = new Random();
        for (int i = 0; i < TEST_SET_SIZE; i++) {
            final Calendar c = Calendar.getInstance();
            c.setTimeInMillis(Math.floorMod(r.nextLong(1, m), m));
            array[0][i] = new MyDate(c, true);
            array[1][i] = new MyDate(c, false);
        }
        return array;
    }
    ...
    public static TestSet<MyDate> createTestSet(int i, int j, int k, int l, MyDate[][] testData) {
        ...
        if (j == 1) {
            ...
            map = new MyListsHashMap<>(new Hash2IndexFct<>(l == 1 ? 3 * TEST_SET_SIZE : (int) (0.1 * TEST_SET_SIZE), 0));
    ...
    public static void test(TestSet<MyDate> testSet) {
        final MyDate[] array = testSet.getTestData();
        final MyMap<MyDate, MyDate> map = testSet.getHashTable();
        for (int i = 0; i < 750; i++) {
            final MyDate m = array[i];
            map.put(m, m);
        }
